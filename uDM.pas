...
uses
  ...
  DateUtils, raCodMod, ppBands, ppCache, ppDesignLayer, ppPrnDev, Dialogs;
...
type
  TDM = class(TDataModule)
    ...
    cdsAlmacenNum: TDAMemDataTable;
    dsAlmacenNum: TDADataSource;
  ...
  public
    ...
    function Imprimir_Proxy(CualReporte: String; IdentificadorID: Integer; AParametros, Ruta, TipoArchivo: String; Directo: Boolean): String;
    ...
    function Imprimir(CualReporte: String; IdentificadorID: Integer; AParametros, Ruta, TipoArchivo: String; Directo: Boolean): String;
  end;
...
begin
  ...
  begin
    ...
    case Aux of    //
     ...
     12: Parametros.Almacen:= StrToInt(Valor);
     13: Parametros.Almacen:= StrToInt(Valor);
...
begin
  ...
  NOMBREIMPRESORA:= LeerRegistro('NOMBREIMPRESORA','PRINTER');
end;
...
begin
  ...
  begin
    ...
    lPrinterDevice.Publisher := repReportes.Publisher;
    {set the printer name for the device}
    repReportes.Printer.PrinterName  := NOMBREIMPRESORA;
    {print the report to the devices}
    Ruta:= 'Imprimiendo';
    repReportes.PrintToDevices; //you could also create more than one printer devices to print on two or
  end
  else
  begin
  ...
  if not Directo then
     ...
     on E : Exception do
      Ruta:= E.ClassName+' error raised, with message : '+E.Message;
  end;
end;

function TDM.Imprimir(CualReporte: String; IdentificadorID: Integer;
  AParametros, Ruta, TipoArchivo: String; Directo: Boolean): String;
begin
  try
    try
      Result:=Imprimir_Proxy(CualReporte, IdentificadorID, AParametros, Ruta, TipoArchivo, Directo);
    except
      on E : Exception do
      begin
    ...
    try
      ...
      begin
        ...
        Imprime(cdsReportes.FieldByName('SQL1').AsString,
                    ...
                    Ruta, TipoArchivo, Directo);
        //Error:='';
        Result:=Ruta;
      end
      else
        //Error:='No se encontró el layout del reporte.';
    except
      on e: Exception do
